...
module.exports = function (grunt, fileGroup, browser, options, next) {
  ...
  if (typeof browser === 'string') {
    ...
    driver = new webdriver.Builder().
        usingServer(options.seleniumServer).
  ...
  driver.getSession().then(function(session) {
    ...
    mocha.suite.on('pre-require', function(context, file, m) {
      ...
      this.ctx.file = file;
      this.ctx.baseUrl = options.baseUrl;
      this.ctx.netloc = options.netloc;
      this.ctx.lcpUrl = options.lcpUrl;
      this.ctx.constants = options.constants;

      var Po, pageObject;

      for (pageObject in options.pageObjects) {
        Po = options.pageObjects[pageObject];
        this.ctx[pageObject] = new Po(ptor);
      }
    });
